...
const useUpdatesData = ({ page = 1 } = {}) => {
  ...
  useEffect(() => {
    ...
    const loadUpdates = async () => {
      ...
      try {
        const response = await getCourseUpdatesData(courseId, { page, pageSize: CUSTOM_TAB_PAGE_SIZE });
        if (!cancelled) {
